...
export function Button({ variant = 'primary', size = 'md', loading, disabled, children, className = '', ...props }: ButtonProps) {
  return (
    <button
      className={`font-ui inline-flex items-center justify-center gap-1.5 font-semibold transition-all duration-[var(--duration-fast)] min-h-[44px] ${variants[variant]} ${sizes[size]} ${disabled || loading ? 'opacity-40 cursor-not-allowed' : ''} ${className}`}
