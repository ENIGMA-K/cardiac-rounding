...
import type { RoundingRecord, ClinicalPhase, RoundingSession, TodoItem } from '@/types';
import { db } from '@/lib/db';

interface RoundingState {
  currentRecord: RoundingRecord | null;
  records: RoundingRecord[];
  loading: boolean;
  dirty: boolean;
  error: string | null;

  createRecord: (patientId: string, phase: ClinicalPhase, session: RoundingSession) => Promise<RoundingRecord>;
  ...
  saveRecord: (status?: 'draft' | 'completed') => Promise<void>;
}

export const useRoundingStore = create<RoundingState>((set, get) => ({
  ...
  loadRecord: async (recordId) => {
    ...
    if (record) set({ currentRecord: record, dirty: false });
  },
