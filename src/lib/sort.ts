...
function buildBedRankMap(): Map<string, number> {
  const map = new Map<string, number>();
  let rank = 0;
  const seq: string[] = [];
  for (let i = 1; i <= 18; i++) { seq.push(String(i)); if (i % 3 === 0) { const n = i / 3; seq.push(`+${n}`); seq.push(`Z${n}`); } }
  for (let i = 19; i <= 30; i++) seq.push(String(i));
  seq.push('+10');
  for (let i = 31; i <= 33; i++) seq.push(String(i));
  seq.push('+11');
  for (let i = 34; i <= 39; i++) seq.push(String(i));
  for (let i = 40; i <= 43; i++) seq.push(String(i));
  seq.push('+14'); seq.push('+15');
  for (let i = 44; i <= 46; i++) seq.push(String(i));
  seq.push('+16');
  for (const bed of seq) map.set(bed, rank++);
  for (let i = 1; i <= 60; i++) map.set(`CICU${i}`, rank++);
  for (let i = 1; i <= 30; i++) map.set(`NICU${i}`, rank++);
  return map;
}

const bedRankMap = buildBedRankMap();

function parseBed(bed: string): { prefix: string; num: number } {
  const m = bed.match(/^([A-Za-z+]*)(\d+)$/);
  if (m) return { prefix: m[1].toUpperCase(), num: parseInt(m[2]) };
  return { prefix: '', num: 0 };
}

export function sortByBedNumber(patients: Patient[]): Patient[] {
  return [...patients].sort((a, b) => {
    ...
    const prefixOrder = (p: string) => { if (p === 'CICU') return 0; if (p === 'NICU') return 1; if (p === '') return 2; if (p === '+') return 3; if (p === 'Z') return 4; return 5; };
...
export function computePhaseDays(patient: Patient): PhaseDays {
  const today = new Date(); today.setHours(0, 0, 0, 0);
  if (patient.phase === 'discharged') return { label: '已出院', days: 0 };
  ...
  if (patient.surgeryDate) {
    ...
    if (days === 0) return { label: '手术日', days: 0 };
...
export function sortByPhaseProgress(patients: Patient[]): Patient[] {
  const phaseOrder: ClinicalPhase[] = ['post-op-ward', 'post-op-ward-monitor', 'post-op-icu', 'surgery-day', 'pre-op', 'transfer', 'discharged'];
  return [...patients].sort((a, b) => {
