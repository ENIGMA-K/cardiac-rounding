...
export function getPhaseColor(phase) { return phaseColorMap[phase] || '#94a3b8'; }
export function getPhaseLabel(phase) { return phaseLabels[phase] || phase; }
...
export function sameDay(a, b) { return dateStr(a) === dateStr(b); }
/** Get day-by-day phases for dot rendering */
export function getPatientDays(patient, today) {
        ...
        if (surgeryDay && d >= surgeryDay) {
            if (sameDay(d, surgeryDay)) {
                ...
                for (let h = history.length - 1; h >= 0; h--) {
                    if (history[h].date <= dStr) {
                        found = history[h].phase;
                        break;
                    }
                }
...
export function patientToGanttTasks(patient, today) {
    if (!patient.admissionDate)
        return [];
    const admission = new Date(patient.admissionDate + 'T00:00:00');
    const start = new Date(admission);
    ...
    const days = [];
    let surgeryDay = patient.surgeryDate ? new Date(patient.surgeryDate + 'T00:00:00') : null;
