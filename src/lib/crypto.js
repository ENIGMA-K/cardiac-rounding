...
function toBuf(v) { return v.buffer.slice(v.byteOffset, v.byteOffset + v.byteLength); }
export async function hashPassword(pw) {
    ...
    if (!C?.subtle) {
        ...
        r.set(new TextEncoder().encode(pw));
        r.set(salt, new TextEncoder().encode(pw).length);
        for (let i = 0; i < ITERATIONS / 1000; i++) {
            const mix = new Uint8Array(32);
            for (let j = 0; j < 32; j++)
                mix[j] = (r[j % r.length] + r[(j * 7 + i) % r.length] + i) & 255;
            r = mix;
        }
...
export async function verifyPassword(pw, hash, saltB64) {
